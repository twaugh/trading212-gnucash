...
class Trading212Transaction(BaseModel):
    ...
    action: str = Field(..., alias="Action", description="Transaction type (Market buy, Market sell, etc.)")
    ...
    ticker: Optional[str] = Field(None, alias="Ticker", description="Stock ticker symbol")
    ...
    num_shares: Optional[Decimal] = Field(None, alias="No. of shares", description="Number of shares traded")
    price_per_share: Optional[Decimal] = Field(None, alias="Price / share", description="Price per share")
    price_currency: Optional[str] = Field(None, alias="Currency (Price / share)", description="Currency of share price")
    exchange_rate: Optional[Decimal] = Field(None, alias="Exchange rate", description="Exchange rate used")
    result_currency: Optional[str] = Field(None, alias="Currency (Result)", description="Result currency")
    total: Decimal = Field(..., alias="Total", description="Total transaction amount")
    total_currency: str = Field(..., alias="Currency (Total)", description="Currency of total amount")
    conversion_fee: Optional[Decimal] = Field(None, alias="Currency conversion fee", description="Conversion fee")
    conversion_fee_currency: Optional[str] = Field(None, alias="Currency (Currency conversion fee)", description="Conversion fee currency")
    french_tax: Optional[Decimal] = Field(None, alias="French transaction tax", description="French transaction tax")
    french_tax_currency: Optional[str] = Field(None, alias="Currency (French transaction tax)", description="French tax currency")
    stamp_duty_tax: Optional[Decimal] = Field(None, alias="Stamp duty reserve tax", description="Stamp duty reserve tax")
    stamp_duty_tax_currency: Optional[str] = Field(None, alias="Currency (Stamp duty reserve tax)", description="Stamp duty tax currency")
    ...
    def validate_action(cls, v):
        ...
        if v not in supported_actions:
            raise ValueError(f"Unsupported action type: {v}. Supported: {supported_actions}")
        return v
    
    @field_validator('num_shares', 'price_per_share', 'exchange_rate', 'total', 
                    'conversion_fee', 'french_tax', 'stamp_duty_tax', mode='before')
    @classmethod
    ...
    @field_validator('isin', 'ticker', 'name', 'notes', 'price_currency', 
                    'result_currency', 'total_currency', 'conversion_fee_currency',
                    'french_tax_currency', 'stamp_duty_tax_currency', mode='before')
    @classmethod
...
class GnuCashSplit(BaseModel):
    ...
    transaction_commodity: str = Field(default="", description="Transaction commodity (stock symbol)")
